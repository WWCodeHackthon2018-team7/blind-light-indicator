...
//Indicate which pin on the board
int photocellPin = 0;
long previousMillis = 0; //stores last time state was updated
long interval = 60000; //creates time interval
void setup() {
  Serial.begin(112500);
...
void loop() {
  ...
  if(currentMillis - previousMillis > interval) {
    ...
    //If the the temperature value drops below 5C, close the blinds
    if  { (t < 5)
      //Turn the Servo a full rotation
      angle = 180;
       //OR if the value of the photcell reads below 600 or the temperature is above 31C, close the blinds
    } else if (photoValue > 600 || t > 31) {
        angle = 180;
      } else {
      angle = 0;
      }
